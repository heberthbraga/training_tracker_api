class Api::V1::WorkoutExercisesController < ApplicationController
  ...
  api :POST, '/api/v1/activities/:activity_id/workout_exercises', 'Creates a new workout exercise'
  param :activity_id, Integer, desc: 'id of the activity which will own the workout training', required: true
  param :workout_exercise, Hash, desc: 'Workout Exercise information', required: true do
    ...
    param :muscle, Hash, desc: 'Muscle Group information', required: true do
      param :name, Integer, desc: 'Name of the muscle group required for the exercise', required: true
    end
  end
  def create
